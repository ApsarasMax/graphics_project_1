...
// Each line direction method has an associated constant.
enum
{
	SLIDER = 0,
	BRUSH_DIRECTION,
	NUM_LINE_METHOD // Make sure this stays at the end!
};
...
public:
	...
	//The implementation of all brushes using right click
	virtual void RightBrushBegin( const Point source, const Point target ) = 0;
	virtual void RightBrushMove( const Point source, const Point target ) = 0;
	virtual void RightBrushEnd( const Point source, const Point target ) = 0;
	...
	static int			c_nLineMethodCount;	// How many brushes we have,
	static ImpBrush**	c_pLineMethods;		// and what they are.
