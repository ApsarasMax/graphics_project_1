...
ImpressionistDoc::ImpressionistDoc() 
{
	...
	// create one instance of each line direction method
	ImpBrush::c_nLineMethodCount	= NUM_LINE_METHOD;
	ImpBrush::c_pLineMethods	= new ImpBrush* [ImpBrush::c_nLineMethodCount];

	ImpBrush::c_pLineMethods[SLIDER]	= new SingleLineBrush( this, "Slider" );
	ImpBrush::c_pLineMethods[BRUSH_DIRECTION]	= new SingleLineBrush( this, "Brush Direction" );

		ImpBrush::c_pLineMethods[SLIDER]	= new ScatteredLinesBrush( this, "Slider" );
	ImpBrush::c_pLineMethods[BRUSH_DIRECTION]	= new ScatteredLinesBrush( this, "Brush Direction" );

	// make one of the line direction current
	m_pCurrentLineMethod	= ImpBrush::c_pLineMethods[0];
...
//---------------------------------------------------------
// Called by the UI when the user changes the line direction method.
// type: one of the defined line direction methods.
//---------------------------------------------------------
void ImpressionistDoc::setLineMethod(int type)
{
	m_pCurrentLineMethod = ImpBrush::c_pLineMethods[type];
}
...
//---------------------------------------------------------
// Returns the line width of the single line brush.
//---------------------------------------------------------
...
//---------------------------------------------------------
// Returns the angle of the single line brush.
//---------------------------------------------------------
...
//---------------------------------------------------------
// Sets the angle of the right click line brush.
//---------------------------------------------------------
...
//---------------------------------------------------------
// Returns the value of the filter matrix.
//---------------------------------------------------------
double ImpressionistDoc::getVal()
{
	return m_pUI->getVal();
}

//---------------------------------------------------------
// Returns the value of the filter divisor.
//---------------------------------------------------------
double ImpressionistDoc::getDivisor()
{
	return m_pUI->getDivisor();
}

//---------------------------------------------------------
// Returns the value of the filter offset.
//---------------------------------------------------------
double ImpressionistDoc::getOffset()
{
		...
		double divisor, double offset )
{
	...
	for (int j = 0; j < srcBufferHeight*3; j++ )
	{
		for (int i = 0; i < srcBufferWidth; i+=3 )
		{
			//destBuffer[i+j*srcBufferWidth] = (int)sourceBuffer[i+j*srcBufferWidth], 255;

			destBuffer[i+j*srcBufferWidth] = min((int)sourceBuffer[i+j*srcBufferWidth] * *filterKernel / divisor + offset, 255);
			destBuffer[i+j*srcBufferWidth+1] = min((int)sourceBuffer[i+j*srcBufferWidth+1] * *filterKernel / divisor + offset, 255);
			destBuffer[i+j*srcBufferWidth+2] = min((int)sourceBuffer[i+j*srcBufferWidth+2] * *filterKernel / divisor + offset, 255);
...
// Get the min number
//----------------------------------------------------------------
int ImpressionistDoc::min(int a, int b){
	if(a>b) return b;
	return a;
}
